...
// Cambiar a 'http://localhost:3000' si tu backend no usa el prefijo /api
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        alert("Ruta no encontrada en el servidor (404).");
      }
    } catch (error) {
    ...
    try {
      ...
      if (contentType && contentType.includes('application/json')) {
        ...
        if (response.ok) {
          ...
          alert(`Error al registrar la solicitud: ${data.error || 'intenta de nuevo'}`);
        }
      } else {
        alert("Ruta /registrar-solicitud no encontrada en el backend (404). Revisa las rutas de Express.");
      }
    } catch (error) {

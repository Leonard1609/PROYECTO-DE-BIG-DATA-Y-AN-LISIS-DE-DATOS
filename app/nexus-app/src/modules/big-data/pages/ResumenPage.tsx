...
import { rubroLabel } from '../services/catalog';
import { explainWinner } from '../services/explain';
import { rankSameRubro } from '../services/recommend';
import { supabase } from '../../../shared/lib/supabaseClient';

const formatMoney = (value: number) =>
  new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 }).format(value);

const financials = <T extends object>(dataset: T) => {
  const values = dataset as Record<string, unknown>;
  const toNumber = (value: unknown) => (typeof value === 'number' ? value : Number(value) || 0);
  const ingresos = toNumber(values.ingresos ?? values.ingreso ?? values.ventas);
  const costos = toNumber(values.costos ?? values.costo);
  const gastos = toNumber(values.gastos ?? values.gasto);
  const existing = values.gananciaNeta;

  return {
    basis: existing !== undefined || ingresos || costos || gastos ? 'dinero' : 'sin_dinero',
    gananciaNeta: existing === undefined ? ingresos - costos - gastos : toNumber(existing),
  } as const;
};

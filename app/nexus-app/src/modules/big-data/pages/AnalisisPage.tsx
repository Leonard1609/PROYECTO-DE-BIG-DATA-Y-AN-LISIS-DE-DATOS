...
import { FinancialDistributionChart } from '../charts/FinancialDistributionChart';
...
const REPORT_INSIGHT_KEY = 'nexus-report-insight';
...
const ComparisonTrendChart: React.FC<{
  labels: string[];
  metricLabel: string;
  series: TrendSeries[];
}> = ({ labels, metricLabel, series }) => {
  const values = series.flatMap((item) => item.values);
  const max = Math.max(...values.map((value) => Math.abs(value)), 1);

  return (
    <div className="space-y-3" aria-label={`Gráfico de ${metricLabel}`}>
      {series.map((item) => (
        <div key={item.label}>
          <div className="flex items-center gap-2 text-xs nx-muted mb-1">
            <span className="h-2 w-2 rounded-full" style={{ backgroundColor: item.color }} />
            {item.label}
          </div>
          <div className="flex items-end gap-1 h-32">
            {item.values.map((value, index) => (
              <div key={`${item.label}-${labels[index] ?? index}`} className="flex-1 h-full flex items-end" title={`${labels[index] ?? ''}: ${value}`}>
                <div
                  className="w-full rounded-t-sm min-h-1"
                  style={{ height: `${Math.max((Math.abs(value) / max) * 100, 2)}%`, backgroundColor: item.color }}
                />
              </div>
            ))}
          </div>
          <div className="flex gap-1 mt-1 text-[10px] nx-muted">
            {labels.map((label) => <span key={label} className="flex-1 text-center truncate">{label}</span>)}
          </div>
        </div>
      ))}
    </div>
